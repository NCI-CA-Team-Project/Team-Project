...
import com.lingo.lingoalpha1_0.service.MessageService;
...
//sending a message
    ...
    public ResponseEntity<Message> sendMessage(@PathVariable Long receiverId, @RequestBody String messageContent, @AuthenticationPrincipal UserDetails userDetails){
        User sender = userRepository.findByUserName(userDetails.getUsername())
                .orElseThrow();

        Message message = messageService.sendMessage(sender.getId(), receiverId, messageContent);
